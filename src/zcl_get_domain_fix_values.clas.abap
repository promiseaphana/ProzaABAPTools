...
CLASS zcl_get_domain_fix_values IMPLEMENTATION.
  ...
  METHOD if_rap_query_provider~select.
    ...
    TRY.
        ...
        IF line_exists( filter_condition_ranges[ name = 'DOMAIN_NAME' ] )."filter_condition_domain_name IS NOT INITIAL.
          domain_name = filter_condition_ranges[ name = 'DOMAIN_NAME' ]-range[ 1 ]-low.
        ELSE.
...
*        DATA(values) = xco_cp_domain=>text_attribute->for( CONV #(  domain_name ) ).
*        DATA(lo_domain) = xco_cp_domain=>origin( iv_name = 'Z_DOMAIN' ).
*        DATA(ls_domain_content) = lo_domain->content(
*           iv_origin = xco_cp_domain=>origin->default
*        )->get( ).
